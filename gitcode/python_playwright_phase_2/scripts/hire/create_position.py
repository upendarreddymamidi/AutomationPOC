...
async def run_script(page, expect, row, env, user_id, country, idx):
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Availability Date"][:2])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Availability Date"][2:4])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Availability Date"][4:])
        await page.wait_for_timeout(3000)

        # Fill in "Earliest Hire Date"
        await page.get_by_role("spinbutton", name="Month", exact=True).nth(1).click()
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Earliest Hire Date"][:2])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Earliest Hire Date"][2:4])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Earliest Hire Date"][4:])
        await page.wait_for_timeout(3000)
