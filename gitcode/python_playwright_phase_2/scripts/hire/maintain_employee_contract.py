...
async def run_script(page, expect, row, env, user_id, country, idx):
    if row["Want to Rescind?"] == "Yes" or row["Want to correct?"] == "Yes":
        return True
    if country not in ["United States of America", "Japan"]:
        ...
        try:
            ...
            if row["Contract End date"] != "nan":
                await page.get_by_role("spinbutton", name="Month").nth(3).click()
                await page.wait_for_timeout(3000)
                await page.keyboard.type(row["Contract End date"][:2])
                await page.wait_for_timeout(3000)
                await page.keyboard.type(row["Contract End date"][2:4])
                await page.wait_for_timeout(3000)
                await page.keyboard.type(row["Contract End date"][4:])
                await page.wait_for_timeout(3000)
        ...
        safe_print("Skipped because of country conditions of", country)
        return True
