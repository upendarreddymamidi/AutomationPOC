...
async def run_script(page, expect, row, env, user_id, country, idx):
    ...
    try:
        ...
        if row["Pay Rate Type"] != "Hourly" and (
            row["Salary Amount"] != "nan"
            or row["Salary Currency"] != "nan"
            or row["Salary Frequency"] != "nan"
            or row["Salary Compensation Plan"] != "nan"
        ):
            if await page.get_by_role(
                "button", name="Edit Salary", exact=True
            ).is_visible():
                await page.get_by_role("button", name="Edit Salary", exact=True).click()
            else:
                await page.get_by_role("button", name="Add Salary").click()
                await page.wait_for_timeout(3000)
            ...
            if row["Salary Frequency"] != "nan":
                ...
                await page.wait_for_timeout(3000)
                await page.keyboard.type(row["Salary Frequency"])
                await page.wait_for_timeout(3000)
                await page.keyboard.press("Enter")
                await page.wait_for_timeout(3000)
                await page.keyboard.press("Enter")
        ...
        if row["Pay Rate Type"] == "Hourly" and (
            ...
            if await page.get_by_role("button", name="Delete Salary").is_visible():
                await page.wait_for_timeout(3000)
                await page.get_by_role("button", name="Delete Salary").click()
            ...
            if await page.get_by_role(
                "button", name="Edit Hourly", exact=True
            ).is_visible():
                await page.wait_for_timeout(3000)
                await page.get_by_role("button", name="Edit Hourly", exact=True).click()
                await page.wait_for_timeout(3000)
            else:
                await page.wait_for_timeout(3000)
                ...
                if row["Hourly Compensation Plan"] != "nan":
                    await page.get_by_role("textbosx", name="Compensation Plan").fill(
            ...
            if row["Hourly Frequency"] != "nan":
                await page.get_by_role("option", name="press delete to").last.click()
                await page.wait_for_timeout(3000)
                await page.keyboard.type(row["Hourly Frequency"])
                await page.wait_for_timeout(3000)
                await page.keyboard.press("Enter")
                await page.wait_for_timeout(3000)
                await page.keyboard.press("Enter")
