...
async def run_script(page, expect, row, env, user_id, country, idx):
    ...
    try:
        ...
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Effective Date for Rescind"][:2])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Effective Date for Rescind"][2:4])
        await page.wait_for_timeout(3000)
        await page.keyboard.type(row["Effective Date for Rescind"][4:])
        await page.wait_for_timeout(3000)
        await page.get_by_role("button", name="Filter", exact=True).click()
        await page.wait_for_timeout(3000)

        await page.get_by_text("Leave Request").first.click()
