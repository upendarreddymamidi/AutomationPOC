...
async def run_script(page, expect, row, env, user_id, country, idx):
    ...
    try:
        ...
        if "promotion" in row["Why making this changes?"].lower():
            process = "Promotion"
        elif "lateral" in row["Why making this changes?"].lower():
            process = "Lateral Move"
        ...
        if page.get_by_text("skipped").is_visible():
            ...
            if await popup_header_selector.is_visible():
                ...
                await page.screenshot(
                    path=f"{os.getenv('SCREENSHOT_FOLDER')}/{user_id}/success/job_change_edit_additional_data_2_{idx}.png"
                )
        ...
        if await page.is_visible(error_widget_selector) and not (
            ...
            await page.wait_for_timeout(5000)
            await page.screenshot(
                path=f"{os.getenv('SCREENSHOT_FOLDER')}/{user_id}/error/job_change_edit_additional_data_2_{idx}.png"
            )
